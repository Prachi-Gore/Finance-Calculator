...
server = function(input, output) {
  ...
  output$dynamic_ui <- renderUI({
    ...
    if (input$radio == 1) {
      ...
      column(width=8,sub_ui("Present Value"))
    } else if (input$radio == 2) {
      
      column(width=8,sub_ui("Future Value"))
    } else {
    ...
    result <- switch(input$radio,
                     ...
                     "3" = switch(input$time,
                                "End"=switch(input$payable,
                                             ...
                                             "Half Yearly" =  pmt(r = nominal_rate(effective_rate(input$rate,2),12)/12, n = 12*input$nper, pmt = input$pmt, fv = 0,type=0),
                                             "Monthly" =  pmt(r = input$rate/12, n = 12*input$nper, pmt = input$pmt, fv = 0,type=0)#monthly interest given and monthly installment asked
                                             ),
                                "Begining"= switch(input$payable,
                                                    "Yearly" =  pmt(r = nominal_rate(input$rate,12)/12, n = 12*input$nper, pv = input$pmt, fv=0,type=1),
                                                    "Half Yearly" =  pmt(r = nominal_rate(effective_rate(input$rate,2),12)/12, n = 12*input$nper, pmt = input$pmt, fv = 0,type=1),
                                                    "Monthly" =  pmt(r = input$rate/12, n = 12*input$nper, pmt = input$pmt, fv = 0,type=1)#monthly interest given and monthly installment asked
                                )
    ))
